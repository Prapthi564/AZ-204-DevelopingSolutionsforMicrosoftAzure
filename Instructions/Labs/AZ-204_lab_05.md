...
1.  Run the following command to create a new **virtual machine**:
    ...
    ```
    az vm create --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name quickvm --image Debian11 --admin-username student --admin-password StudentPa55w.rd
    ```
...
1.  Run the following command to get a more detailed JSON file that contains various metadata about the newly created VM:
    ...
    ```
    az vm show --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name quickvm
    ```    
1.  Run the following command to list all the IP addresses associated with the VM:

    ```
    az vm list-ip-addresses --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name quickvm
    ```
    
1.  Run the following command to filter the output to only return the first IP address value:

    ```
    az vm list-ip-addresses --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name quickvm --query '[].{ip:virtualMachine.network.publicIpAddresses[0].ipAddress}' --output tsv
    ```    
1.  Run the following command to store the results of the previous command in a new Bash shell variable named *ipAddress*:

    ```
    ipAddress=$(az vm list-ip-addresses --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name quickvm --query '[].{ip:virtualMachine.network.publicIpAddresses[0].ipAddress}' --output tsv)
    ```    
...
1. At the **Cloud Shell** command prompt in the portal, run the following command to create a Container Registry resource: 
    ...
    ```bash
    az acr create --resource-group ContainerCompute-<inject key="DeploymentID" enableCopy="false"/> --name $registryName --sku Basic
    ```    
...
1.  On the **Access Keys (1)** blade, in the **Admin user** section, select the **checkbox (2)**.
...
1.  On the **Create container instance** blade, perform the following actions, and then select **Create (10)**:

    | Setting                            | Action                       |
    | ---------------------------------- | ---------------------------- |
    | **Container name** text box        | Enter **managedcompute** (1)    |
    | **Container image** text box       | Retain the default value. (2)    |
    | **OS type** section                | Select **Linux** (3)            |
    | **Subscription** text box          | Retain the default value. (4)    |
    | **Resource group** drop-down list  | Select **ContainerCompute-<inject key="DeploymentID" enableCopy="false"/>** (5) |  
    | **Location** drop-down list        | Select **East US**  (6)       |
    | **Number of cores** drop-down list | Select **2** (7)               |
    | **Memory (GB)** text box           | Enter **4** (8)               |
    | **Public IP address** section      | Select **No** (9)               |

    ![Create container instance blade](./media/az204-create-1.png)
...
## Exercise 4: Create a secure Container Apps environment and deploy container app
...
1. Set environment variables used later in this exercise. 
   ...
   ```bash
     myRG=ContainerCompute-<inject key="DeploymentID" enableCopy="false"/>
     myAppContEnv=az204-env-<inject key="DeploymentID" enableCopy="false"/>
    ```

1. In the **Cloud Shell** command prompt in the portal, run the following command in **Bash** window to create an environment by using the `az containerapp env` create command: 
...
1. In the **Cloud Shell** command prompt in the portal, run the following command to deploy a sample app container image by using the `az containerapp create` command: 
    ...
    ```
   >**Note: By setting `--ingress` to `external`, you make the container app available to public requests. The command returns a link to access your app.

1. Select the link returned by the `az containerapp create` command to verify the container app is running.

### Review

In this exercise, you have enabled the admin user in Container Registry, automatically and manually deployed a container image to an Azure container instance and validated the container instances ran successfully.
