...
1. If you are not logged in already, click on the **Azure portal** shortcut that is available on the desktop and log in with Azure credentials.
    ...
    ![](./media/azure-portal-shortcut-1505.png)
...
1. On the **Create a storage account** blade, on the **Basics** tab, perform the below actions and click on **Next (7)** to go to **Advanced** tab. 
   ...
   | **Storage account name** text box | Enter **securestor<inject key="DeploymentID" enableCopy="false"/> (3)** |
   | **Region** drop-down list         | Select **(US) Central US (4)**.                                     |
   ...
   ![Screenshot displaying the configured settings on the Create a storage account blade](./media/create-storage-account-1505.png)
...
1. In the **Access keys** section, select **Show** option for the keys and connection string.
...
3. On the **Create key vault** blade, on the **Basics** tab, perform the following actions, and then select **Access Configuration** tab:
   ...
   | **Key vault name** text box       | Enter **securevault<inject key="DeploymentID" enableCopy="false"/> (3)**        |
   | **Region** drop-down list         | Select **Central US (4)**.                       |
   | **Pricing tier** drop-down list   | select **Standard (5)**.                      |

   The following screenshot displays the configured settings on the **Create key vault** blade.

   ![Screenshot displaying the configured settings on the Create key vault blade](./media/a11.png)
...
In this exercise, you created the resources Storage account, Azure Key Vault and Function app that you'll use in this lab.
...
1. On the **Create a secret** blade, perform the following actions, and then select **Create (5)**:

   | Setting                            | Action                                             |
   | ---------------------------------- | -------------------------------------------------- |
   | **Upload options** drop-down list | Select **Manual (1)**. |
   | **Name** text box       | Enter **storagecredentials (2)**                                  |
   | **Secret value** text box       | Enter the storage account connection string **(3)** that you recorded previously in this lab.               |
   | **Content type** text box | Leave blank. |
   | **Set activation date** check box | Not selected. |
   | **Set expiration date** check box | Not selected. |
   | **Enabled** option | Select **Yes (4)**. |

   The following screenshot displays the configured settings on the **Create a secret** blade.

    ![Screenshot displaying the configured settings on the Create a secret blade ](./media/create-secret-1505.png)
...
1. On the **Create an access policy** blade, select **1 Permission (1)** section and make the following selections then click **Next (3)**:

   | Setting                            | Action                                             |
   | ---------------------------------- | -------------------------------------------------- |
   | **Configure from template** drop-down list | Leave blank. |
   | **Key permissions** drop-down list | 0 selected.      |
   | **Secret permissions** drop-down list | Select the **GET (2)** permission. |
   ...
   The following screenshot displays the configured settings in the **Create an access policy** blade.
    ...
    ![Screenshot depicting the configured settings on the Add access policy blade.](./media/a13.png)

1. On the **Principal** page, make the following selections:
...
1. At the bottom of the **App settings** section, select **Apply** then **Confirm** to save the changes.
...
1. Run the following command to upgrade to the latest version of **Azure Functions Core Tools**.
    ...
    ```
    > **Note:** This might take a few minutes to execute successfully. 
...
1. Open the **Visual Studio Code** app from Desktop.
...
1. Run the following command to test the **GET** REST API call against `http://localhost:7071/api/FileParser`:
...
1. In the Sign in window that pops up, select Work or school accont and click on COntinue. 

1. On the **Microsoft** sign in window, enter the **email address** and **password** for your Microsoft account provided in the Environment tab of this lab, and then select **Sign in**.

    > **Note:** On the *Automatically sign in to all desktop apps and website on this device?* Select **No, this app only**.

1. Return to the currently open **Command Prompt** window. Wait for the sign-in process to finish.

1. On Select a Subscription and Tenant, type 1 and press enter.
...
1. **Run** the following command to **publish** the function app project:
    ...
    > **Note**: **Replace** the above placeholder with the **Funtion App Name** that we created earlier in this lab : **securefunc<inject key="DeploymentID" enableCopy="false"/>**. For example, the final command would be ``func azure functionapp publish securefunc9234``.
...
1. Go back to the **Microsoft Edge** where you are logged in to the **Azure portal** (<https://portal.azure.com>).
...
1. The following screenshot displays the output.
    ...
    ![Output of the code.](./media/a23.png)
...
1. In the **New container** pop-up window, perform the following actions, and then select **Create**:
   ...
   | **Anonymous access level** drop-down list | Select **Blob (anonymous read access for blobs only)**. |
     ...
     >**Note**: If you are not able to change the access level Go to the configuration in the settings of the storage account and enable "allow blob anonymous access" and save then return to the container and try again.
...
1. In the **Change access level** pop-up window, perform the following actions:
    ...
    - In the **Anonymous access level** drop-down list, select **Private (no anonymous access)**.
...
1. In the **Microsoft** window, enter the email address and password for your Microsoft account, and then select **Sign in**.

1. Type 1 to select subscription and press Enter. 
