...
1. In the **Authentication (1)** section, perform the following actions, and select **Configure (5)**:
...
1. Back in the **Platform** Configurations section, select **Add URI (6)**, and then enter `https://localhost:5001/signin-oidc` **(7)**.

1. In the **Implicit Grant and hybrid flows** section, select **ID tokens (used for implicit and hybrid flows) (8)**.

1. Select **Save (9)**.

    The following screenshot displays the configured settings on the **Authentication** blade.
          
    ![Screenshot displaying the options configured on the Authentication blade.](./media/az204135.png)

    ![Screenshot displaying the options configured on the Authentication blade.](./media/az204136.png)

    ![Screenshot displaying the options configured on the Authentication blade.](./media/az204137.png)
...
1. Navigate to the **Properties** folder, open the **launchSettings.json** file, and then apply the following changes:
    ...
    >**Note:** Do not modify the application URL within the profiles.
...
1. Review the **Welcome** home page of the target site displayed by the browser and verify that the UPN of the **aad_user1** Entra ID account appears in the browser window.
