...
1. On the **Create Web App** blade, on the **Basics** tab, perform the following actions, and then select the **Next: Database (10)** tab:

    
    | Setting                         | Action                                                       |
    | ------------------------------- | ------------------------------------------------------------ |
    | **Subscription**  | Retain the default value **(1)**                             |
    | **Resource group** drop-down list      | Select **ApiService-<inject key="DeploymentID" enableCopy="false"/> (2)**. |
    | **Name** text box               | Enter **httpapi<inject key="DeploymentID" enableCopy="false"/> (3)**|
    | **Secure unique default hostname** | **Disabled (4)** |
    | **Publish** section             | Select **Container (5)**                              |
    | **Operating System** section    | Select **Linux (6)**                                       |
    | **Region** drop-down list       | **West US (7)** |
    | **Linux Plan** section          | Select **Create new**, enter  the value **ApiPlan** in the **Name** text  box, and then select **OK (8)**. |
    | **Pricing Plan** section    | Select **Explore Pricing plan**, on the **Select App service Pricing Plan** blade, select **Standard S1** under Production then click on **Select (9)**. 
    
    ![](./media/3012025(7).png) 

1. Keep the Database tab at its default settings, then click **Next: Container**.
...
1. Within the web application, perform the following actions:
    ...
    b. Select **GET /status/{codes} (1)**.

    c. Select **Try it out**.

    d. In the **codes** text box, enter **404 (2)**.
...
1. On the **Create API Management service** blade, perform the following actions, and then select **Review + create**:
    ...
    | --------------------------------- | ------------------------------------------------------------ |
    | **Subscription**     | Retain the default value **(1)**                           |
    | **Resource group** drop-down list        | Select the **ApiService-<inject key="DeploymentID" enableCopy="false"/> (2)** group|
    | **Region** list     | Select the same region you chose in the previous exercise **(3)** |
    | **Resource name** text box        | Enter **proapi<inject key="DeploymentID" enableCopy="false"/> (4)** |
    | **Organization name** text box    | Enter **Contoso (5)**                                           |
    | **Administrator email** text  box | Enter `admin@contoso.com` **(6)**                                    |
    | **Pricing tier** drop-down  list   | **Consumption (99.95% SLA) (7)**                                |
...
1. In the **Create an HTTP API** window, perform the following actions and select **Create (4)**:
    
    | Setting | Action |
    | -- | -- |
    | **Display name** text box | Enter **HTTPBin API (1)** |
    | **Name** text box | Enter **httpbin-api (2)** |
    | **Web service URL** text box | Enter the URL for the web app that you copied earlier in this lab. **Note**: Make sure that the URL starts with the https:// prefix **(3)** |
...
1. In the **Add operation** section, perform the following actions, and then select **Save (4)**:

    | Setting | Action |
    | -- | -- |
    | **Display name** text box | Enter **Echo Headers (1)** |
    | **Name** text box | Verify that its value is set to **echo-headers (2)** |
    | **URL** list | Select **GET** |
    | **URL** text box | Enter **/ (3)** |
...
1. Back on the **Design** tab, in the list of operations, select **Echo Headers (1)**.

1. In the **Design** section, on the **Inbound processing** tile select **+ Add policy (2)**.
...
1. Back on the **Design** tab, in the list of operations, select **Echo Headers**, and then select the **Test (1)** tab.

1. In the **Echo Headers** section, select **Send (2)**.
...
1. Select the **Design** tab, and then select **Get Legacy Data (1)**.

1. On the **Design** pane, in the **Outbound processing** section, select **+ Add policy (2)**.
