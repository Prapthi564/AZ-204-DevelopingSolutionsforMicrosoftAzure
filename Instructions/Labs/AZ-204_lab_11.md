...
In this lab, you will create an Application Insights resource in Azure to monitor and log application insight data for later review. The API will be set to automatically scale if demand increases to a certain threshold, and logging the data will help determine how the service is being utilized.
...
To install the **C#** extension for this lab, follow the steps below in Visual Studio Code:
...
4. After installing C# extensions, close Visual Studio Code.
...
1. If you are not logged in already, click on the **Azure portal** shortcut on the desktop and log in with your Azure credentials.
...
1. On the **Application Insights** blade, on the **Basics** tab, perform the following actions, and select **Review + Create (7)**:
    
    | Setting                         | Action                                                       |
    | ------------------------------- | ------------------------------------------------------------ |
    | **Subscription** drop-down list | Retain the **default value (1)**                                    |
    | **Resource group** section      | Select Existing **MonitoredAssets-<inject key="DeploymentID" enableCopy="false"/> (2)** |
    | **Name** text box     | **instrm<inject key="DeploymentID" enableCopy="false"/> (3)** |
    | **Region** drop-down list       | Select the Azure **region in which your Resource group deployed (4)** |
    | **WORKSPACE DETAILS** section | Retain the default values for the **Subscription (5)** and **Log Analytics Workspace (6)** drop-down lists|
    
    The following screenshot displays the configured settings on the **Application Insights** blade.

    ![Create an Azure Application Insights instance blade](./media/a27.png)
...
1. In the **AutoScale setting** section, perform the following actions, and then select **Save**:
    ...
    | Setting                         | Action                                                       |
    | ------------------------------- | ------------------------------------------------------------ |
    | **Scale out** section | Select **Custom autoscale (1)**|
    | **Autoscale setting name** text box     | Enter **ComputeScaler (2)**|
    | **Resource group** drop-down list     |Select **MonitoredAssets-<inject key="DeploymentID" enableCopy="false"/>** |
    | **Scale mode** section      | Select **Scale based on a metric (3)** |
    | **Minimum ** text box in the **Instance limits** section | Enter **1 (4)** |
    | **Maximum** text box in the **Instance limits** section | Enter **8 (4)** |
    | **Default** text box in the **Instance limits** section | Enter **3 (4)** |
    | **Rules** section |  Select **Add a rule (5)** |
    | **Scale rule** blade | Retain default values for all settings, and then select **Add (6)** |
...
1. At the terminal prompt, **run** the following command to **create** a new .NET Web API application named **SimpleApi** in the current directory:
   ...
   > **Note:** If any **error** occurs regarding the framework, try installing .NET 8.0 again using the below commands
...
1. At the **terminal prompt**, **run** the following command to launch the .NET Web API.
    ...
    dotnet run
    ```
    > **Note**: Sometimes you will be getting two port numbers. Record any of the **Port numbers** mentioned in your Output display of Visual Studio Code. For example, as shown in the image below.
...
1. On the lab computer, **switch** to Visual Studio Code.
...
1. **Run** the following command to display the list of web apps in the **MonitoredAssets-<inject key="DeploymentID" enableCopy="false"/>** resource group, whose names start with **smpapi\***:
...
1. **Run** the following commands to display the name of the first web app identified in the previous step and store it in a variable named **$webAppName**:
...
1. On the **App Service** blade, select **Application Insights (1)** under **Monitoring**.

1. On the **Application Insights** blade, perform the following actions, select **Apply (8)**, and then in the confirmation dialog, select **Yes**:

    | Setting                         | Action                                                       |
    | ------------------------------- | -------------------------------|
    | **Application Insights** slider | Ensure it is set to **Enable (2)** |
    | **Instrument your application** section    | Select the **.NET (3)** tab |
    | **Collection level** section    | Select **Recommended (4)**  |
    | **Profiler** section      | Select **On (5)** |
    | **Snapshot debugger** section | Select **Off (6)** |
    | **SQL Commands** section | Select **Off (7)** |
    
    The following screenshot displays the **Application Insights** settings of the Azure Web API.
     
     ![Application Insights settings of the Azure Web API](./media/a28.png)
